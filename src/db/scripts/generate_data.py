...
def gen_products():
    ...
    for _ in range(int(sys.argv[1])):
        result = CREATIVE_LLM.invoke(dedent('''
            ...
            name (string), description (string), rating (float in the range [0,1]), price (float), discount (float in the range [0,1]), category (string in ["Electronics", "Clothes", "Accessories", "Furniture"]), owner (string), reviews (empty array).
